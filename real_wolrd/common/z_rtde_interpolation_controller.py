...
from common.shared_memory_dp.shared_memory_queue2 import (
...
class RTDEInterpolationController(mp.Process):
    ...
    def start_wait(self):
        ...
        if not ready:
            if self.is_alive():
                raise RuntimeError(
                    f"RTDE controller for {self.robot_ip} did not become ready "
                    f"within {self.launch_timeout} seconds."
                )
            raise RuntimeError(
                f"RTDE controller for {self.robot_ip} exited before it became ready."
            )
        if not self.is_alive():
            raise RuntimeError(
                f"RTDE controller for {self.robot_ip} exited before it became ready."
        ...
        try:
            ...
            if self.payload_mass is not None:
                if self.payload_cog is not None:
                    ...
                    assert rtde_c.setPayload(self.payload_mass,[0,0,0])
            ...
            phase = "startup" if not self.ready_event.is_set() else "runtime"
            message = (
                f"RTDE controller for {robot_ip} failed during {phase}: {e}\n"
            ...
            if rtde_c is not None:
                try:
                    ...
                    if self.verbose:
                        print(f"[RTDEPositionalController] servoStop failed: {e}")
                ...
                try:
                    ...
                    if self.verbose:
                        print(f"[RTDEPositionalController] stopScript failed: {e}")
                rtde_c.disconnect()
